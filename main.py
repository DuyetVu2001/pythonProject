...
class DataProcessorApp:
    ...
    def process_data(self):
        if self.df is not None:
            try:
                self.df['DateOfBirth'] = pd.to_datetime(self.df['DateOfBirth'], errors='coerce')
                ...
                most_frequent_dob = self.df['DateOfBirth'].mode().dropna().iloc[0]
                self.df['DateOfBirth'] = self.df['DateOfBirth'].fillna(most_frequent_dob)
                ...
                self.df['DateOfBirth'] = self.df['DateOfBirth'].apply(parse_date)
    ...
    def clean_data(self):
        if self.df is not None:
            try:
                ...
                self.df.drop_duplicates(subset=['Name', 'DateOfBirth','JoinDate'], keep='first', inplace=True)
